...
namespace CharStream{
    ...
    function msgpack_pack($in){
        MsgPack::Shim()->msgpack_pack($in);
    }

    function msgpack_unpack(){
        MsgPack::Shim()->msgpack_unpack();
    }

    class MsgPack {

        const ENGINE_UNSET  = 0x00;
        const ENGINE_NATIVE = 0x01;
        const ENGINE_PHP    = 0x02;

        protected static $self = null;
        protected static $type = self::ENGINE_UNSET;
        protected static $_nsp = ["PHP","Native","PHP"];


        public function __construct (){
            self::$type = extension_loaded("msgpack") ? self::ENGINE_NATIVE : self::ENGINE_PHP;

            if(self::$type === self::ENGINE_PHP)
                throw new \Exception("Msgpack running in capiblity mode. Please install msgpack-php",E_WARNING);
        }
        ...
        public function msgpack_pack($in){
            $nsc  = self::$_nsp[self::$type];
            $nsc .= "\\".__METHOD__;
            return $nsc($in);
        }

        public function msgpack_unpack(){
            $nsc  = self::$_nsp[self::$type];
            $nsc .= "\\".__METHOD__;
            return $nsc();
        }
...
namespace CharStream\MsgPack\Native {

}


namespace CharStream\MsgPack\PHP {
    /**
